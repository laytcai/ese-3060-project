...
@torch.no_grad()
def prediction_confidence(logits):
    """
    Compute per-example confidence:
    - max_prob: max softmax probability
    - margin:   top1 - top2 softmax probability
    """
    probs = F.softmax(logits, dim=1)
    top2 = torch.topk(probs, 2, dim=1).values  # [N, 2]
    max_prob = top2[:, 0]
...
@torch.no_grad()
def infer_mirror_translate_batch(inputs, net):
    """
    Batch version of the original infer_mirror_translate:
    6 effective views per image:
      - mirror / no-mirror
      - +/- 1px translations with mirror / no-mirror
    Uses the same weighting as in airbench94.
    """
    pad = 1

    # Base: mirror + no-mirror
    logits_base = 0.5 * net(inputs) + 0.5 * net(inputs.flip(-1))

    # Two translated crops
    ...
    for tv in translated_views:
        logits_trans += 0.5 * net(tv) + 0.5 * net(tv.flip(-1))
    logits_trans /= len(translated_views)  # average over the 2 translations

    # Combine base and translated logits (same 0.5 / 0.5 weighting as original)
    return 0.5 * logits_base + 0.5 * logits_trans


@torch.no_grad()
def evaluate_dynamic(model, loader, tau=0.80, margin_thresh=0.20):
    """
    Dynamic-TTA evaluation:

    1) Run a single 1-view pass over all test images.
    2) Compute confidence + margin for each image.
    3) For "hard" images (low confidence / small margin), rerun with full
       mirror+translate TTA; for "easy" images, keep the 1-view logits.
    """
    model.eval()

    # Same data access pattern as infer()
    test_images = loader.normalize(loader.images)  # [N, 3, 32, 32], already on GPU
    labels      = loader.labels                    # [N], already on GPU
    N = labels.size(0)

    # -------- Stage 1: 1-view logits for all test images --------
    logits1_list = []
    for inputs in test_images.split(2000):
        logits1_list.append(model(inputs))
    logits1 = torch.cat(logits1_list, dim=0)       # [N, 10]

    # Confidence & margin from 1-view logits (use float32 for stability)
    max_prob, margin = prediction_confidence(logits1.float())
    hard_mask = (max_prob < tau) | (margin < margin_thresh)  # [N] bool
    easy_mask = ~hard_mask

    # Start with 1-view logits as default
    logits = logits1.clone()

    # -------- Stage 2: full TTA only on "hard" examples --------
    if hard_mask.any():
        hard_idx  = hard_mask.nonzero(as_tuple=False).squeeze(1)  # [H]
        hard_imgs = test_images[hard_idx]                         # [H, 3, 32, 32]

        logits_hard_list = []
        for inputs in hard_imgs.split(2000):
            logits_hard_list.append(infer_mirror_translate_batch(inputs, model))
        logits_hard = torch.cat(logits_hard_list, dim=0)          # [H, 10]

        logits[hard_idx] = logits_hard

    # -------- Final accuracy & simple stats --------
    preds = logits.argmax(dim=1)
    acc = (preds == labels).float().mean().item()

    frac_hard = hard_mask.float().mean().item()
    avg_forwards = 1.0 + 6.0 * frac_hard   # 1 view for everyone + 6 TTA views for hard ones
    print(f"[Dynamic TTA] frac_hard={frac_hard:.3f}, avg_forwards_per_image={avg_forwards:.2f}")
...
def main(run):
    ...
    starter.record()
    tta_val_acc = evaluate_dynamic(
    model,
    test_loader,
    tau=0.80,          # you can tune these
    margin_thresh=0.20
    )
    ender.record()
    torch.cuda.synchronize()
