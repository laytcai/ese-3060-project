...
def prediction_confidence(logits):
    """Return max-probability and margin between top two classes."""
    probs = F.softmax(logits, dim=1)
    top2 = torch.topk(probs, 2, dim=1).values   # shape [batch, 2]
    max_prob = top2[:, 0]
    margin   = top2[:, 0] - top2[:, 1]
    return max_prob, margin

def tta_views(x):
    """Return the 6 TTA augmentations used in airbench94."""
    # x: shape [1, 3, 32, 32]
    views = []
    flips = [x, torch.flip(x, dims=[3])]  # original + mirrored along width

    # translations of ±1 pixel horizontally
    for img in flips:
        views.append(img)
        views.append(torch.roll(img, shifts=1, dims=3))
        views.append(torch.roll(img, shifts=-1, dims=3))
