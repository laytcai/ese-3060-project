...
def batch_crop(images, crop_size, shifts):
    r = (images.size(-1) - crop_size)//2

    # shifts are now passed in as an argument, removing the randomness here.
...
class CifarLoader:
    ...
    def __iter__(self):
        ...
        if self.aug.get('translate', 0) > 0:
                    # Deterministic cropping logic (Alternating Translation)
                    # Sequence: Center, Top-Left, Bottom-Right, Top-Right, Bottom-Left
                    r = self.aug['translate']
                    shift_sequence = torch.tensor([
                        [0, 0], [-r, -r], [r, r], [-r, r], [r, -r]
                    ], device=self.images.device)
                    
                    # Select shift based on current epoch modulo 5
                    current_shift = shift_sequence[self.epoch % 5]
                    
                    # Apply to all images in batch
                    shifts = current_shift.repeat(len(self.images), 1)
                    
                    images = batch_crop(self.proc_images['pad'], self.images.shape[-2], shifts=shifts)
