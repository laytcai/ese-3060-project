...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        # ---- forward the GPT model as usual ----
        x = self.transformer.wte(idx)  # (B, T, C)

        # we'll capture hidden states at layer 4 and 8
        h4 = None
        h8 = None

        for i, block in enumerate(self.transformer.h):
            x = block(x)
            # i is 0-based: i==3 -> 4th block, i==7 -> 8th block
            if i == 3:
                h4 = x
            elif i == 7:
                h8 = x
        ...
        if targets is not None:
            ...
            if self.training:
                # ===== heavy deep supervision (training only) =====

                # normalize intermediate states the same way as final
                h4_ln = F.rms_norm(h4, (h4.size(-1),))
                h8_ln = F.rms_norm(h8, (h8.size(-1),))

                logits4 = self.lm_head(h4_ln).float()  # (B, T, V)
                logits8 = self.lm_head(h8_ln).float()  # (B, T, V)
                ...
                # same weights as the original slow version
                lambda4 = 0.1
                lambda8 = 0.1
                ...
                # EVAL MODE: report ONLY the baseline loss
                loss = loss_main
